...
it("saves all changed detail fields together before importing", async () => {
  ...
  const save = vi.spyOn(orderImportApi, "saveLines").mockRejectedValueOnce(new Error("offline"))
    .mockResolvedValueOnce({ ...candidate, version: 2, contractShipDates: ["2026-09-12"], shippedQuantity: 25, pendingQuantity: 75, lines: [{ ...candidate.lines[0], factoryName: "测试工厂B", contractShipDate: "2026-09-12", shippedQuantity: 25, pendingQuantity: 75 }] });
  ...
  await saveButton.trigger("click");
  await flushPromises();
  expect(save).toHaveBeenLastCalledWith("candidate-1", { version: 1, lines: [{ candidateLineId: 1, factoryId: "factory-b", contractShipDate: "2026-09-12", shippedQuantity: 25 }] });
