    ...
    <article class="order-workspace order-form-page">
      ...
      <form class="section-card order-form" @submit.prevent="save">
        <section><header class="section-heading"><h2>基本信息</h2></header><div class="form-grid">
          ...
          <label>合同出货时间<span class="date-summary">{{ dateSummary }}</span></label>
        </div></section>
          ...
          <article v-for="(line, lineIndex) in form.lines" :key="line.key" class="product-line-card">
            ...
            <div class="assignment-list">
              <div v-for="(assignment, assignmentIndex) in line.assignments" :key="assignment.key" class="assignment-row">
                ...
                <label>合同出货时间<input v-model="assignment.contractShipDate" type="date" /></label>
...
const form = reactive({ orderNo: "", orderDate: "", tracker: "", lines: [{ key: key(), variantId: "", orderQuantity: 1, assignments: [] }] as LineForm[] });
const addLine = () => form.lines.push({ key: key(), variantId: "", orderQuantity: 1, assignments: [] });
const addAssignment = (line: LineForm) => line.assignments.push({ key: key(), factoryId: "", quantity: 1, contractShipDate: "" });
const assigned = (line: LineForm) => line.assignments.reduce((sum, item) => sum + (Number(item.quantity) || 0), 0);

const dateSummary = computed(() => [...new Set(form.lines.flatMap((line) => line.assignments.map((item) => item.contractShipDate)).filter(Boolean))].sort().join("、") || "—");

async function save() {
  errorMessage.value = "";
  if (!form.orderNo || (!isEdit && !form.orderDate) || !form.tracker || form.lines.some((line) => !line.variantId || !Number.isInteger(line.orderQuantity) || line.orderQuantity <= 0)) { errorMessage.value = "请完整填写基本信息、产品规格和正整数订单数量"; return; }
  ...
  const payload = { orderNo: form.orderNo, tracker: form.tracker as "烧麦", lines: form.lines.map((line) => ({ variantId: line.variantId, orderQuantity: line.orderQuantity, assignments: line.assignments.map((item) => ({ factoryId: item.factoryId, quantity: item.quantity, contractShipDate: item.contractShipDate || null })) })) };
  saving.value = true;
  try {
