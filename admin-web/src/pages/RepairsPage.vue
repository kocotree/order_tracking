  ...
  <AdminShell title="返修退回">
    <article class="order-list-page repair-list-page">
        ...
        <form class="order-filter-form" @submit.prevent="search">
          <div class="order-filter-row repair-filter-row">
            <label class="order-list-search-field">
              <span class="sr-only">搜索返修周期或工厂名称</span>
              <svg viewBox="0 0 24 24" fill="none" aria-hidden="true"><circle cx="11" cy="11" r="6.5"/><path d="m16 16 4 4"/></svg>
              <input v-model="keyword" type="search" placeholder="输入返修周期或工厂名称" autocomplete="off" />
            </label>
        ...
        <div class="table-scroll">
          <table class="orders-table repair-list-table data-grid-table">
            ...
            <tbody>
              ...
              <tr v-if="!loading&&!pageItems.length"><td colspan="9"><div class="empty-state"><div><span class="empty-state-mark">0</span><strong>没有符合当前条件的返修单</strong><p>可以调整返修周期、状态、工厂或周期后重新查询。</p></div></div></td></tr>
            </tbody>
          </table>
        </div>
        <footer class="order-list-footer repair-list-footer">
          <span>每页展示 10 条返修周期。</span>
      ...
      <div v-if="archiveTarget" class="detail-confirm-layer">
        <button class="detail-confirm-backdrop" type="button" aria-label="取消归档返修周期" @click="archiveTarget=null"></button>
        <section class="detail-confirm-dialog order-delete-dialog" role="dialog" aria-modal="true" aria-labelledby="repair-archive-title" aria-describedby="repair-archive-description">
          <h2 id="repair-archive-title">归档返修周期</h2>
          <p id="repair-archive-description">确认归档返修周期 <strong>{{ archiveTarget.repairNo }}</strong>？归档后管理员和工厂小程序均不再显示。</p>
...
type SortField = "repairNo" | "factoryName" | "repairedQuantity" | "scrappedQuantity" | "returnedQuantity" | "warehouseReturnQuantity" | "status";
const sortableColumns: Array<{ label:string; field:SortField }> = [
  { label:"返修周期", field:"repairNo" }, { label:"工厂", field:"factoryName" },
...
const keyword=ref(""), status=ref("all"), factoryFilter=ref<string[]>([]), period=ref(""), periods=ref<string[]>([]);
...
watch([keyword, status, factoryFilter, period, sortBy, sortOrder], () => { page.value=1; }, {deep:true, flush:"sync"});
watch([keyword, status, factoryFilter, period, sortBy, sortOrder, page], () => { void load(); }, {deep:true});
async function load(){
  const id=++requestId;
  loading.value=true; error.value="";
  try {
    const result=await repairApi.listSummaries({keyword:keyword.value,status:status.value,factories:[...factoryFilter.value],period:period.value,sortBy:sortBy.value,sortOrder:sortOrder.value,page:page.value,pageSize});
