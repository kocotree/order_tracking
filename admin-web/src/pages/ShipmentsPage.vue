      ...
      <section class="order-list-filter-card" aria-label="发货单筛选">
        <form class="order-filter-form" @submit.prevent="search">
          <div class="order-filter-row shipment-filter-row">
            ...
            <label class="order-select-field shipment-receipt-field"><span class="sr-only">收货状态</span><select v-model="receiptStatus" aria-label="收货状态"><option value="">全部收货状态</option><option value="RECEIVED">已收货</option><option value="UNRECEIVED">未收货</option></select></label>
...
const receiptLabels = { RECEIVED: "已收货", UNRECEIVED: "未收货" } as const;
const pageItems = ref<ShipmentSummary[]>([]); const total = ref(0); const factories = ref<string[]>([]);
const keyword = ref(""); const factoryNames = ref<string[]>([]); const factorySearch = ref(""); const factoryOpen = ref(false); const receiptStatus = ref<"" | "RECEIVED" | "UNRECEIVED">(""); const dateFrom = ref(queryDate(route?.query.dateFrom)); const dateTo = ref(queryDate(route?.query.dateTo)); const loading = ref(true); const error = ref(""); const optionsError = ref(""); const page = ref(1); const pageSize = 10; const sortKey = ref<SortKey | null>(null); const sortDirection = ref<"asc" | "desc">("asc");
