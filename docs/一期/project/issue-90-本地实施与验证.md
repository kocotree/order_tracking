...
本地工作区为 `/Users/wxl/Desktop/kk/order_tracking-issue-90`，分支 `codex/issue-90-detail-dispatch`，前置提交 `f10a203`（#89）。本轮仅实现 #90 的明细分批派工、工厂发货闭环、派工通知和订单详情表交互；#91 的整单完成、按工厂撤回及配套口径未提前实现。
...
- #91：整单完成、按工厂撤回、合同与提醒等配套口径及四项整体回归；
- 真实飞书来源读取、真实通知外发；
- 微信开发者工具和真机验收；
