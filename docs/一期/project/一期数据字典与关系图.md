...
## Issue #111 采购订单关联字段（2026-09-16）

迁移 `20260916_0040` 在 `order_details` 增加以下可空字段；两者只保存已确认来源快照中的精确采购关联，不进行猜测。飞书订单确认导入及未派工来源更新确认同步维护字段，手工订单、无匹配或非法快照值保持 `NULL`。

| 字段 | MySQL 类型 | 可空 | 来源与边界 |
|---|---|---|---|
| `purchase_order_id` | `TEXT` | 是 | 聚水潭采购主订单号；来自 `accepted_raw_fields._purchase.mainOrderId` 的非空字符串 |
| `purchase_order_item_id` | `TEXT` | 是 | 聚水潭采购子订单号；来自 `accepted_raw_fields._purchase.childOrderId` 的非空字符串 |

迁移只对历史合法字符串去除首尾空白后回填，缺失、空字符串和非字符串不回填。`accepted_raw_fields._purchase` 原始快照继续完整保留，显式字段不替代或改写该快照；本增量不增加页面、API、采购业务表或实时同步。

核对日期：2026-09-08；本地代码基线：`3b2c90d`；业务基线：V1.60。

## Issue #88–#91 本地模型增量（2026-09-11）

本地迁移链至 `20260911_0037`。`order_details` 保存完整来源、原文、人工日期覆盖和当前派工指针；`orders.detail_mode` 区分来源明细协议，`tracker_locked_at` 保留首派跟单人锁定。`order_change_previews` 保存来源／派工预览与版本，幂等记录保存请求与结果。

assignment 新增可空 `detail_id` 和 `is_active`。来源与 assignment 不再是历史一对一：同来源可保留多条失效历史，但最多一条当前有效执行关联；普通 `detail_id` 索引取代唯一索引，事务以订单／来源行锁及当前指针保证一致性。行＋工厂唯一约束已改普通索引，允许同 SKU／工厂不同来源分批派工；初始已发可超出派工量。

未派工读已接受来源值，已派工读固定初始基线加有效账本；逐条待发下限零再求和，未知保持空值。撤回保留所有原 ID、数量账本和合同，只失效所选厂派工并清除来源当前指针；历史草稿关联标为 inactive。新增来源／派工／撤回数据继续受有损降级保护。

本段替代下文旧模型快照相关约束及 latest revision 描述；迁移和历史保留证据见 [#91 实施记录](issue-91-本地实施与验证.md)，不代表生产实查。
