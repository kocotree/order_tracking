...
export interface ShipmentFile { draftVersion?:number|null; fileId:number; filename:string; mimeType:string; sizeBytes:number; contentSha256:string; displayOrder:number; contentUrl:string }
...
export interface Shipment { withdrawalDraftId?:string|null; canEditWithdrawal?:boolean; operations?:{action:string;reason:string;actorName:string;createdAt:string}[]; receipt?: {status:string;confirmedByName:string|null;confirmedAt:string|null}|null; receiptDifferences?:ShipmentLine[]; version:number; shipmentId:string; shipmentNo:string|null; status:string; factoryId:string; factoryName:string; createdBy:string; preferredOrderId:string|null; businessDate:string|null; note:string; totalBoxes:number; totalQuantity:number; lines:ShipmentLine[]; boxes:ShipmentBox[]; files:ShipmentFile[]; voidRequest?:ShipmentVoidRequest|null; returnEvents?:ShipmentReturnEvent[]; createdAt:string; submittedAt:string|null }
export interface CatalogItem { assignmentId:number; orderId:string; orderNo:string; contractShipDate:string | null; productName:string; propertiesValue:string; assignedQuantity:number; shippedQuantity:number; pendingQuantity:number }
...
function parseUploadResponse(result: WechatMiniprogram.UploadFileSuccessCallbackResult): ShipmentFile {
  ...
  throw Object.assign(new Error(payload.message || "发货凭证上传失败"), {statusCode:result.statusCode});
}
...
export const shipmentApi = {
  ...
  currentDraft: () => authorizedRequest<Shipment>({url:"/factory/shipments/drafts/current",method:"GET"}).catch(error => { if (error.statusCode === 404) return null; throw error; }),
  abandonDraft: (shipmentId:string,version:number) => authorizedRequest<WechatMiniprogram.IAnyObject>({url:`/factory/shipments/drafts/${encodeURIComponent(shipmentId)}?version=${version}`,method:"DELETE"}),
  saveDraft: (shipmentId:string, boxes:DraftBoxWrite[], note:string, version=1) => authorizedRequest<Shipment>({url:`/factory/shipments/drafts/${encodeURIComponent(shipmentId)}`,method:"PUT",data:{boxes,note,version}}),
  uploadFile: (shipmentId:string, filePath:string, idempotencyKey:string, onProgress:(progress:number)=>void, version?:number) => new Promise<ShipmentFile>((resolve,reject) => {
    const task = wx.uploadFile({
      url:`${currentApiBaseUrl()}/factory/shipments/drafts/${encodeURIComponent(shipmentId)}/files${version === undefined ? "" : `?version=${version}`}`,
      filePath,
      name:"file",
  ...
  removeFile: (shipmentId:string,fileId:number,version?:number) => authorizedRequest<{version?:number}>({url:`/factory/shipments/drafts/${encodeURIComponent(shipmentId)}/files/${fileId}${version === undefined ? "" : `?version=${version}`}`,method:"DELETE"}),
