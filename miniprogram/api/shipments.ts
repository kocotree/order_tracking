...
export const shipmentApi = {
  ...
  submitDraft: (shipmentId:string,version?:number) => authorizedRequest<Shipment>({url:`/factory/shipments/drafts/${encodeURIComponent(shipmentId)}/submit${version === undefined ? "" : `?version=${version}`}`,method:"POST",header:{"Idempotency-Key":`shipment-${Date.now()}`}}),
