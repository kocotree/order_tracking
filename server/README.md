...
未显式启用本地演示模式时，飞书、短信、微信、聚水潭和私有文件存储适配器保持禁用，不会静默调用外部服务。
...
管理员网页端点击“通过飞书登录”后会进入只在 `local_demo` 下注册的身份选择页。演示申请人使用不可真实投递的合成号码 `10000000000`，验证码为 `123456`。该模式使用假飞书、假短信、假微信和内存头像存储，并显式写入 12 条六分类启用假产品用于验收只读列表；这些记录不是真实聚水潭数据。本地演示仅用于本机验收，不要用于共享测试或生产环境。
