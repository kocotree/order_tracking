...
管理员网页端点击“通过飞书登录”后会进入只在 `local_demo` 下注册的身份选择页。演示申请人的假飞书资料包含不可真实投递的合成号码 `10000000000`，申请页只展示脱敏号码并直接提交，不发送短信。该模式使用假飞书、假微信和内存头像存储，并显式写入 12 条六分类启用假产品用于验收只读列表；这些记录不是真实聚水潭数据。本地演示仅用于本机验收，不要用于共享测试或生产环境。
