...
def _clean_import_data(engine: Engine) -> None:
    with Session(engine) as session, session.begin():
        ...
        session.execute(delete(AuditLog).where(AuditLog.action.like("order_import.%")))
        session.execute(delete(OrderImportRun))
        session.execute(delete(ProductVariant).where(ProductVariant.variant_id == "variant-import"))
...
def test_run_aggregates_ready_candidate_and_keeps_shipped_order_invalid(
    test_database_engine: Engine,
) -> None:
    _clean_import_data(test_database_engine)
    _seed_import_dependencies(test_database_engine)
    ...
    rows = [
        SourceOrderRow(
            "rec-ready", "E100", "6970000000001", "测试童帽", "蓝色 / 120",
            "童帽春夏", "测试工厂", 100, 0, 100, "松子",
            date(2026, 8, 22), date(2026, 8, 30), {},
        ),
        SourceOrderRow(
            "rec-shipped", "E101", "6970000000001", "测试童帽", "蓝色 / 120",
            "童帽春夏", "测试工厂", 80, 20, 60, "松子",
            date(2026, 8, 22), date(2026, 8, 30), {},
        ),
        SourceOrderRow(
            "rec-finished", "E102", "6970000000001", "测试童帽", "蓝色 / 120",
            "童帽春夏", "测试工厂", 50, 50, 0, "松子",
            date(2026, 8, 22), date(2026, 8, 30), {},
        ),
        SourceOrderRow(
            "rec-missing-order", None, "6970000000001", "测试童帽", "蓝色 / 120",
            "童帽春夏", "测试工厂", 50, 0, 50, "松子",
            date(2026, 8, 22), date(2026, 8, 30), {},
        ),
    ]
    ...
    with Session(test_database_engine) as session:
        ...
        assert candidates["E101"].validation_state == "INVALID"
        assert "ALREADY_SHIPPED" in candidates["E101"].validation_issues
        line = session.query(OrderImportCandidateLine).filter_by(
            candidate_id=candidates["E100"].candidate_id
        ).one()
...
def test_worker_retries_before_releasing_failed_import_run(
    ...
    run = service.create_or_reuse_run(
        actor_id="admin-order-import", request_id="retry-run"
    )
    ...
    next_run = service.create_or_reuse_run(
        actor_id="admin-order-import", request_id="after-retry"
    )
